...
名称は **FOSS + Sight** を組み合わせた造語で、OSS を「見る・俯瞰する・監視する」という役割を表します。同時に **Foresight（先を見通す）** の響きも重ねています。正式なカタカナ読みは **フォサイト** です。詳細は `docs/NAMING.ja.md` を参照してください。
...
Windows配布版の変更可能データは `%LOCALAPPDATA%\FossightData` に保存され、アプリ本体の `%LOCALAPPDATA%\Fossight` とは分離されています。詳しい操作は `docs/FOSSIGHT_USER_GUIDE.ja.md`、privacy/securityは `docs/FOSSIGHT_SECURITY_PRIVACY.ja.md` を参照してください。
...
UIではrepository検索、status/relation filter、summary count、update check、OSSごとのEnable/Disableを利用できます。登録済みOSSの日本語説明は `docs/oss-summary-catalog.ja.md` に保持しています。repositoryへhoverまたはkeyboard focusすると短いtooltipを表示し、行を選択すると説明全文とstatus metadataを右側のDetails paneへ固定表示します。DetailsからGitHub homepageも開けます。狭い画面ではDetailsはslide-in drawerへ切り替わります。DisableしたOSSはregistryには残りますがGitHub update checkから除外されます。
